...
let mode = "detail"
let state_edit_mode_selection = document.getElementById("state_edit_mode_selection") as HTMLInputElement
...
type state_public_detail_type = {add_claim?:string|string[],add_homeland?:string|string[]}
type state_private_detail_type = {state_type?:"corporated"|"incorporated"|"treaty_port"|("corporated"|"incorporated"|"treaty_port")[]}
...
const state_detail_edit = (info:[string,string]) => {
    ...
    let index = -1
    state_public.innerHTML = `<p>${info[0]}.region_state:${info[1].replace("c:","")}</p><p>${localization.state_detail_public}:<button>+</button></p>`
    state_private.innerHTML = `<p>${localization.state_detail_private}:<button>+</button></p>`
    if (state_public.classList.contains("state_property_flow")) state_public.classList.remove("state_property_flow")
    if (state_private.classList.contains("state_property_flow")) state_private.classList.remove("state_property_flow")

    if (state_detail["add_claim"]){
        if (!(state_detail["add_claim"] instanceof Array)) state_public_detail["add_claim"] = [state_detail["add_claim"]]
        else state_public_detail["add_claim"] = state_detail["add_claim"]
    }
    if (state_detail["add_homeland"]){
        if (!(state_detail["add_homeland"] instanceof Array)) state_public_detail["add_homeland"] = [state_detail["add_homeland"]]
    ...
    spu_btn.addEventListener("click",function(e){state_public.appendChild(draw_state_detail("","",state_detail,false))})
    spr_btn.addEventListener("click",function(e){state_private.appendChild(draw_state_detail("","",state_split_detail,false))})

    if (state_split_detail["state_type"]){
        if (!(state_split_detail["state_type"] instanceof Array)) state_private_detail["state_type"] = [state_split_detail["state_type"]]
        else state_private_detail["state_type"] = state_split_detail["state_type"]
    }
    ...
    if (state_public_detail["add_homeland"]){
        ...
        state_public.appendChild(draw_state_detail("add_homeland",state_public_detail["add_homeland"][i],state_detail))
    }

    if (state_public_detail["add_claim"]){
        for (let i=0;i<state_public_detail["add_claim"].length;i++)
        state_public.appendChild(draw_state_detail("add_claim",state_public_detail["add_claim"][i],state_detail))
    }
    
    if (state_private_detail["state_type"]){
        for (let i=0;i<state_private_detail["state_type"].length;i++)
        state_private.appendChild(draw_state_detail("state_type",state_private_detail["state_type"][i],state_split_detail))
    }
...
const draw_state_detail = (key:string,value:string,data:state_public_detail_type|state_private_detail_type,disabled=true) => {
...
const state_pops_edit = (info) => {
    ...
    if (!full_map_data.pops_map["POPS"][info[0]][info_detail]["create_pop"]) full_map_data.pops_map["POPS"][info[0]][info_detail]["create_pop"] = {}
    ...
    if (state_public.classList.contains("state_property_flow")) state_public.classList.remove("state_property_flow")
    if (!state_private.classList.contains("state_property_flow")) state_private.classList.add("state_property_flow")

    let spr_btn = state_private.querySelector("button") as HTMLButtonElement
    spr_btn.addEventListener("click",function(e){state_private.appendChild(draw_pop_detail("","",0,pop_detail,"",false))})
    
    if (pop_detail["create_pop"] instanceof Array){
        for (let i=0;i<pop_detail["create_pop"].length;i++){
            state_private.appendChild(draw_pop_detail(pop_detail["create_pop"][i]["culture"],pop_detail["create_pop"][i]["religion"],pop_detail["create_pop"][i]["size"],pop_detail,pop_detail["create_pop"][i]["pop_type"]))
        }
    } else {
        state_private.appendChild(draw_pop_detail(pop_detail["create_pop"]["culture"],pop_detail["create_pop"]["religion"],pop_detail["create_pop"]["size"],pop_detail,pop_detail["create_pop"]["pop_type"]))
    }
...
interface pop_detail {culture:string,size:number,religion:string,pop_type:string}

const draw_pop_detail = (cu:string,religion:string,size:number,data:any,pop_type:string,disabled=true) => {
    let dom = document.createElement("div")
    let parentNode = dom.parentNode as HTMLDivElement
    let minus = btn_adjustment("-")
    dom.appendChild(minus)
    ...
    plus.onclick = function(e) {
        ...
        if (data["create_pop"]["pop_type"]&&pt_input.value || pt_input.value) arr["pop_type"] = pt_input.value
        ...
        if (data["create_pop"] instanceof Array){
            for (let i=0;i<data["create_pop"].length;i++){
                if (data["create_pop"][i]["culture"] == cu_input.value){
                        procees = true
                        if (!data["create_pop"][i]["religion"] && !r_input.value && !pt_input.value && !data["create_pop"][i]["pop_type"]){
                            push = false
                        ...
                        if (data["create_pop"][i]["religion"] != r_input.value || data["create_pop"][i]["pop_type"] != pt_input.value){
                            push = true
                            continue
                        }
                        if (data["create_pop"][i]["religion"] == r_input.value && data["create_pop"][i]["pop_type"] == pt_input.value){
...
const state_building_edit = (info) => {
    ...
    if (!full_map_data.buildings_map["BUILDINGS"][info[0]][info_detail]["create_building"]) full_map_data.buildings_map["BUILDINGS"][info[0]][info_detail]["create_building"] = {}
    let building_detail = full_map_data.buildings_map["BUILDINGS"][info[0]][info_detail]
    state_public.innerHTML = ""
    state_private.innerHTML = `<p>${info[0]}.${info_detail}</p><p>${localization.building}:<button>+</button></p>`
    console.log(building_detail)

    let spr_btn = state_private.querySelector("button") as HTMLButtonElement
    spr_btn.addEventListener("click",function(e){state_private.appendChild(draw_building_detail("","","",[],building_detail,false))})
    if (state_public.classList.contains("state_property_flow")) state_public.classList.remove("state_property_flow")
    if (!state_private.classList.contains("state_property_flow")) state_private.classList.add("state_property_flow")
    ...
    if (building_detail["create_building"] instanceof Array){
        for (let i=0;i<building_detail["create_building"].length;i++){
            state_private.appendChild(draw_building_detail(building_detail["create_building"][i]["building"],
        ...
        state_private.appendChild(draw_building_detail(building_detail["create_building"]["building"],
...
const draw_building_detail = (key:string,lv:string,re:string,pm:string,data:any,disabled=true) => {
    let is_more = false
        ...
        if (data["create_building"] instanceof Array){
            for (let i=0;i<data["create_building"].length;i++){
                ...
                if (data["create_building"][i]["building"] == key_input.value){
                    push = false
                    if (data["create_building"][i]["activate_production_methods"]){
                        arr["activate_production_methods"] = data["create_building"][i]["activate_production_methods"]
                    }
                    data["create_building"][i] = arr
                    parentNode.appendChild(draw_building_detail("","","",data["create_building"][i],data,false))
                    return
                }
            }
        } else{
            data["create_building"] = [arr]
            parentNode.appendChild(draw_building_detail("","","",data["create_building"][0],data,false))
        }

        if (push){
            data["create_building"].push(arr)
            let length =data["create_building"].length -1
            parentNode.appendChild(draw_building_detail("","","",data["create_building"][length],data,false))
        }
    ...
    more.onclick = function(e) {
        ...
        let more_info_btn = more_info.querySelector("button") as HTMLButtonElement
        more_info_btn.addEventListener("click",function(e){more_info.appendChild(draw_pm_detail("",pm,"activate_production_methods"))})

        for (let i=0;i<pm["activate_production_methods"].length;i++){
            more_info.appendChild(draw_pm_detail(pm["activate_production_methods"][i],pm,"activate_production_methods"))
        }
...
const draw_pm_detail = (key,data,kw="") => {
    ...
    minus.onclick = function(e) {
        ...
        data[kw] = data[kw].filter((items:string,index:number) => ![key].includes(items))
...
const state_aresource_edit = (info) => {
    ...
    state_public.innerHTML = `<p>${info[0]}.region_state:${info[1].replace("c:","")}</p><p>${localization.aresource_full}:<button>+</button></p>`
    state_private.innerHTML = ""
    if (!state_public.classList.contains("state_property_flow")) state_public.classList.add("state_property_flow")
    if (state_private.classList.contains("state_property_flow")) state_private.classList.remove("state_property_flow")

    let spu_btn = state_public.querySelector("button") as HTMLButtonElement
    spu_btn.addEventListener("click",function(e){state_public.appendChild(draw_pm_detail("",state_detail,"arable_resources"))})
...
const state_cresource_edit = (info) => {
    let state_detail = full_map_data.state_regions_map[info[0].replace("s:","")]
    state_public.innerHTML = `<p>${info[0]}.region_state:${info[1].replace("c:","")}</p><p>${localization.cresource_full}:<button>+</button></p>`
    state_private.innerHTML = ""
    if (!state_public.classList.contains("state_property_flow")) state_public.classList.add("state_property_flow")
    if (state_private.classList.contains("state_property_flow")) state_private.classList.remove("state_property_flow")
    ...
    if (state_detail["capped_resources"]){
        for (let i=0,keys=Object.keys(state_detail["capped_resources"]);i<keys.length;i++){
            let key = keys[i]
            state_public.appendChild(draw_kv_detail(key,state_detail["capped_resources"][key],state_detail))
        }
    }
}
...
const draw_resource_detail = (key,akey,un,max,data,index=0) => {
    ...
    plus.onclick = function(e) {
        ...
        let res = {"type":key_input.value,"undiscovered_amount":parseInt(un_input.value)}
        if (akey_input.value) res["depleted_type"] = akey_input.value
        if (max_input.value) res["discover_amount_max"] = parseInt(max_input.value)
...
const state_resource_edit = (info) => {
    ...
    let spu_btn = state_public.querySelector("button") as HTMLButtonElement
    spu_btn.addEventListener("click",function(e){state_public.appendChild(draw_resource_detail("","","","",state_detail,-1))})
    ...
    if (state_detail["resource"]){
        if (state_detail["resource"] instanceof Array) {
            for (let i=0;i<state_detail["resource"].length;i++){
                ...
                state_public.appendChild(draw_resource_detail(

...
let strategic_name_input = document.getElementById(
    "strategic_name_input"
) as HTMLInputElement;
let culture_name_input = document.getElementById(
    "culture_name_input"
) as HTMLInputElement;
let capital_name_input = document.getElementById(
    "capital_name_input"
) as HTMLInputElement;
...
var reset_data = ctx.getImageData(
    0,
    0,
    canvas.width,
    canvas.height
...
const gethexname: hexnamer = (r, g, b) =>
    "x" +
    (
...
var history_state_dict: history_dict_type,
    state_regions_map,
...
if (!debug) {
    ...
    strategic_regions_map = await get_text_dict(
        "./data/outputs/02_strategic_regions.txt"
    );
    state_regions_map = await get_text_dict(
        "./data/outputs/01_state_regions.txt"
    );
...
var adjacencies = await fetch("./data/adjacencies.csv")
    .then((resp) => resp.text())
    .then((buffer) => get_csv(buffer));

var adj_pos: (Number | string)[][] = [];
for (let i = 0; i < adjacencies.length; i++) {
    if (adjacencies[i]["start_x"] > 0) {
        adj_pos.push([
            adjacencies[i]["start_x"],
            adjacencies[i]["start_y"],
            adjacencies[i]["stop_x"],
...
var full_map_data = {
    history_state_dict: history_state_dict,
    adj_pos,
    state_regions_map,
    strategic_regions_map,
    pops_map,
...
init_worker.onmessage = function (e) {
    if (e.data["correct_history_state_dict"])
        full_map_data.history_state_dict = e.data["correct_history_state_dict"];
    else if (e.data["localiztion"]) {
        console.log(e.data["localiztion"]);
...
import {
    ...
    state_mode,
    terrain_mode,
} from "./mode_map_render.js";
import { do_draw } from "./drawing_little.js";
import {
    select_provs,
    select_states,
    select_prov_pure,
} from "./canvas_selection.js";
    ...
    if (city_select) {
        ...
        for (
            ...
            if (
                ...
                if (
                    full_map_data.strategic_regions_map[region][
                        "graphical_culture"
                    ]
                )
                    culture_name_input.value =
                        full_map_data.strategic_regions_map[region][
                            "graphical_culture"
                        ];
                if (
...
const canvas_city_select = (label: string, x: number, y: number) => {
    ...
    full_map_data.state_regions_map[state.replace("s:", "")][city_select_type] =
        label;
    do_draw();
};
...
let convert_strategy_btn = document.getElementById(
    "convert_strategy"
) as HTMLButtonElement;

convert_button.onclick = function (e) {
    update_map(
        provs_name,
        state_display_name.value,
        country_display_name.value
    );
};
...
let mode_selection = document.getElementById(
    "mode_selection"

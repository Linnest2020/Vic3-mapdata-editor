...
let river_draw = document.getElementById("river_draw") as HTMLInputElement
let adj_draw = document.getElementById("adj_draw") as HTMLInputElement
let city_draw = document.getElementById("city_draw") as HTMLInputElement
let canvas = document.getElementById("canvas") as HTMLCanvasElement


const do_draw = () => {
    if (river_draw.checked) {
        draw_river()
    }
    if (adj_draw.checked){
        canvas_map_adj()
    }
    if (city_draw.checked) {
        speicail_city()
    }   
}
...
const polygon = (x:number,y:number,radius:number,
    n:number,color:string,scolor="black") => {
...
const speicail_border = (colorname:string,type:string) => {
...
const canvas_draw_adj = (sx:number,sy:number,ex:number,ey:number)=>{
    full_data.ctx.beginPath();

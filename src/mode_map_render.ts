    ...
    if (!full_data.strategic_data || !full_map_data.strategic_data_lock) {
        ...
        for (
            ...
            if (full_map_data.strategic_regions_map[keys[i]]["map_color"]) {
                map_color =
                    full_map_data.strategic_regions_map[keys[i]]["map_color"];
                if (map_color[0] <= 1)
                    map_color = [
                        Math.ceil(map_color[0] * 255),
                        Math.ceil(map_color[1] * 255),
                        Math.ceil(map_color[2] * 255),
                    ];
            ...
            for (
                let j = 0,
                    map_key =
                        full_map_data.strategic_regions_map[keys[i]]["states"],
                    map_len = map_key.length;
                j < map_len;
                j++
            ) {
                if (
                    ...
                    if (
                        !(
                            full_map_data.history_state_dict["STATES"][
                                "s:" + map_key[j]
                            ]["create_state"] instanceof Array
                        )
                    ) {
                        // @ts-ignore
                        colorful_states.push(
                            `s:${map_key[j]}.region_state:${
                                full_map_data.history_state_dict["STATES"][
                                    "s:" + map_key[j]
                                ]["create_state"]["country"]
                            }`
                        );
                    } else {
                        // @ts-ignore
            ...
            for (let m = 0; m < colorful_states.length; m++) {
                let points = full_data.statepointmap[
                    colorful_states[m]
                ] as number[];
                for (let n = 0; n < points.length; n++) {
                    if (
                        (state_data.data[points[n]] << 16) +
                            (state_data.data[points[n] + 1] << 8) +
                            state_data.data[points[n] + 2] >
                        0
                    ) {
...
const country_mode = () => {
    ...
    for (
        let u = 0, len = Object.keys(full_data.statepointmap).length;
        u < len;
        u++
    ) {
        ...
        for (
            let n = 0, nlen = full_data.statepointmap[statepoint].length;
            n < nlen;
            n++
        ) {
            if (
                img_data.data[full_data.statepointmap[statepoint][n]] +
                    img_data.data[full_data.statepointmap[statepoint][n] + 1] +
                    img_data.data[full_data.statepointmap[statepoint][n] + 2] ==
                0
            )
                continue;
            img_data.data[full_data.statepointmap[statepoint][n]] =
                country_color[country][0];
            img_data.data[full_data.statepointmap[statepoint][n] + 1] =
                country_color[country][1];
            img_data.data[full_data.statepointmap[statepoint][n] + 2] =
                country_color[country][2];
        }
    }
...
terrain_worker.onmessage = function (e) {
    if (e.data["localiztion"]) {
        console.log(e.data["localiztion"]);
    } else if (!e.data["ok"]) {
        progress.textContent = e.data["data"];
    } else {
...
const terrain_mode = () => {
    ...
    if (!full_data.terrain_data || !full_map_data.terrain_data_lock) {
        ...
        if (!full_map_data.terrain_map) {
            ...
            get_text_dict("./data/province_terrains.txt").then((res) => {
                full_map_data.terrain_map = res;
            });

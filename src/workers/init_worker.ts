...
var the_length = 0
const initiate = (data:ImageData,histroy_state_dict:{[key:string]:any},width:number) => { 
...
const init_state_map = (colormap:{[key:string]:number[]},histroy_state_dict:{[key:string]:any}) => {
    let nowprogress = 0
    let time = new Date().valueOf()
    for ( let uu=0,len = Object.keys(histroy_state_dict["STATES"]).length;uu<len;uu++){
        let u=Object.keys(histroy_state_dict["STATES"])[uu]
        ...
        if (histroy_state_dict["STATES"][u]["create_state"] instanceof Array){
            ...
            for (let i =0;i<histroy_state_dict["STATES"][u]["create_state"]["owned_provinces"].length;i++){
                ...
                if (!colormap[histroy_state_dict["STATES"][u]["create_state"]["owned_provinces"][i]]){
                    ...
                    histroy_state_dict["STATES"][u]["create_state"]["owned_provinces"] = histroy_state_dict["STATES"][u]["create_state"]["owned_provinces"].filter((items:string,index:number) => ![i].includes(index))
                    continue
                }
